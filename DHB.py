...
riot_api_key = 'RGAPI-0cc8a2fb-264b-42d7-ad67-708f0717eeaf' # api는 자기 라이엇 api사용
...
def dict():
    url='http://ddragon.leagueoflegends.com/cdn/11.23.1/data/ko_KR/champion.json'
    res=requests.get(url).json()
    names=res['data'].keys()
    champ_lst=list(map(lambda x: (res['data'][x]['key'],x),list(res['data'].keys())))
    char_num=[]
    char_name=[]
    for i,j in champ_lst:
        char_num.append(i)
        char_name.append(j)
    kr_names=[]
    for i in names:
        kr_names.append(res['data'][i]['name'])
    kr=pd.Series(kr_names)
    series1=pd.Series(char_num)
    series2=pd.Series(char_name)
    df=pd.DataFrame(series1,columns=['num'])
    df2=pd.DataFrame(series2,columns=['char'])
    df3=pd.concat([df,df2],axis=1)
    df4=pd.DataFrame(kr,columns=['kr'])
